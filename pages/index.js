...
export async function getServerSideProps(context) {
    ...
    } = await axios(process.env.IMDB_API_TITLE)
        .then((response) => (response.data.errorMessage ? titleMock : response.data))
        .catch(() => titleMock)

    const seasonsData = await axios
        .all(seasons.map((season) => axios.get(`${process.env.IMDB_API_SEASONS}${season}`)))
        .then((responses) =>
    ...
    const backgroundImage = await axios(process.env.IMDB_API_IMAGES)
        .then((response) => {
